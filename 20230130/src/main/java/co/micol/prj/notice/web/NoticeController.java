...
//		return "redirect:/noticeList.do";
//	}
	// ▶ @ModelAttribute + alert창 띄우기 -> vo객체의 이름을 그대로 넘겨줘야함 //
	@PostMapping("/noticeEdit.do")
	public String noticeEdit(@ModelAttribute("notice") NoticeVO vo, MultipartFile file, Model model) {
		...
		if (!file.isEmpty()) { // 비어 있으면 true /비어있지않으면 false
			...
			vo.setNoticeFile(file.getOriginalFilename()); // 원본 파일 명
			vo.setNoticeFileDir(saveFolder + fileName);
		}
		int n = noticeService.noticeUpdate(vo);
		if (n != 0) {
			model.addAttribute("message", "정상적으로 수정되었습니다.");
			viewPage = "notice/noticeEdit"; // 돌아갈 페이지
		} else {
